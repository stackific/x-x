...
1. **Which agents?** Multi-select over every registered agent (Claude Code, Codex CLI, OpenCode, GitHub Copilot CLI, Pi today). Blank line / no toggle accepts the default (all agents).
2. **Which scope?** Decides where the bundled SKILLS land. Either way, `.x-plans/` is seeded in the current working directory — that's how the project marker check (`./.x-plans/_config.lock`) recognizes the directory as an x-x project.
   - **This project only** — skills under the current working directory (`.claude/skills/`, `.agents/skills/` shared by Codex, Copilot, and Pi, `.opencode/commands/`).
   - **All my projects (user scope)** — skills under `$HOME` (`~/.claude/skills/`, `~/.agents/skills/` for Codex, Copilot CLI, and Pi, `~/.opencode/commands/`).
