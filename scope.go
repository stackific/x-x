...
// runScopes dispatches `stax scopes <subcommand>`. Future scope-tooling commands
// (e.g. `lint`) can be added here without restructuring, the same way
// `runSkills` is structured.
func runScopes(args []string) {
	if len(args) == 0 {
	...
	switch args[0] {
	case "next-prefix":
		runScopesNextPrefix(args[1:])
	case "list":
		runScopesList(args[1:])
	case "lint":
		runScopesLint(args[1:])
	case "slugify":
		runScopesSlugify(args[1:])
	default:
...
// printPlansUsage writes the `stax scopes` help block to w. Mirrors the
...
func printPlansUsage(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage: stax scopes <subcommand>")
	_, _ = fmt.Fprintln(w, "  next-prefix   Print the next unused zero-padded scope prefix")
	_, _ = fmt.Fprintln(w, "  list          List scopes with slug, status, and declared systems")
	_, _ = fmt.Fprintln(w, "  lint          Validate every scope file against the project schema")
	_, _ = fmt.Fprintln(w, "  slugify       Print the kebab-case slug for a scope title")
}

// runScopesNextPrefix prints the next available zero-padded scope prefix in
...
func runScopesNextPrefix(args []string) {
	os.Exit(planNextPrefix(args, staxDir, os.Stdout, os.Stderr))
}

// planNextPrefix is the testable body of runScopesNextPrefix: flag-set
...
func planNextPrefix(args []string, staxDir string, stdout, stderr io.Writer) int {
	fs := flag.NewFlagSet("scopes next-prefix", flag.ContinueOnError)
	fs.SetOutput(stderr)
	cwdFlag := fs.String("cwd", "", "change to this directory before running (like git -C)")
	fs.Usage = func() {
		_, _ = fmt.Fprintln(stderr, "Usage: stax scopes next-prefix [--cwd PATH]")
	}
	if err := fs.Parse(args); err != nil {
		return 2
	}
	if fs.NArg() > 0 {
		_, _ = fmt.Fprintf(stderr, "stax scopes next-prefix takes no arguments (got %q)\n", fs.Arg(0))
		return 2
	}
...
// the same filename pattern listScopes accepts. Anchoring on `-` and the
...
func scanHighestPrefix(staxDir string, width int) int {
	...
	re := regexp.MustCompile(fmt.Sprintf(`^(\d{%d})-.+%s$`, width, regexp.QuoteMeta(scopeFileExt)))
	highest := 0
...
// runScopesList prints one tab-separated row per scope in staxDir whose
...
//	                                   kebab-case `id:` values that scopes carry
...
//	                                   scopesListOverflowThreshold (see constants.go)
//
...
func runScopesList(args []string) {
	os.Exit(planList(args, staxDir, os.Stdout, os.Stderr))
}

// planList is the testable body of runScopesList. Same exit-code contract
// as the wrapper (0 happy, 1 listScopes IO error, 2 usage / not-a-project
...
func planList(args []string, staxDir string, stdout, stderr io.Writer) int {
	fs := flag.NewFlagSet("scopes list", flag.ContinueOnError)
	fs.SetOutput(stderr)
	...
	cwdFlag := fs.String("cwd", "", "change to this directory before running (like git -C)")
	fs.Var(&statusFlag, "status", "keep only scopes whose status matches (repeatable, comma-separated)")
	fs.Var(&systemFlag, "system", "keep only scopes whose systems contain this id (repeatable; OR semantics; matches the kebab `id:` from _data_systems.yaml)")
	fs.Var(&keywordsFlag, "overflow-keywords", "case-insensitive substring(s) narrowing the output when the post-filter count exceeds scopesListOverflowThreshold (repeatable; OR semantics; matched against scope body only)")
	fs.Usage = func() {
		_, _ = fmt.Fprintln(stderr, "Usage: stax scopes list [--status NAME[,NAME...]] [--system ID] [--order asc|desc] [--overflow-keywords PATTERN[,PATTERN...]] [--cwd PATH]")
	}
	if err := fs.Parse(args); err != nil {
		return 2
	}
	if fs.NArg() > 0 {
		_, _ = fmt.Fprintf(stderr, "stax scopes list takes no positional arguments (got %q)\n", fs.Arg(0))
		return 2
	}
	if err := applyCwd(*cwdFlag); err != nil {
		_, _ = fmt.Fprintln(stderr, "error:", err)
	...
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "stax scopes list: %v\n", err)
		return 2
	}
	...
	width := loadPrefixWidth(staxDir)
	rows, err := listScopes(staxDir, width, stderr)
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "stax scopes list: %v\n", err)
		return 1
	}
	...
	sortScopeRows(filtered, order)
	filtered = applyOverflowNarrow(filtered, keywords, staxDir, scopesListOverflowThreshold)
...
// scopesListOrder enumerates the two values --order accepts. Defined as a
// typed int (not strings) so the sort-by-value switch in sortScopeRows
// can't be invoked with an arbitrary, unvalidated string.
type scopesListOrder int

const (
	orderAsc  scopesListOrder = 1
	orderDesc scopesListOrder = 2
)

// parseOrder validates the --order CLI value and returns its enum.
func parseOrder(s string) (scopesListOrder, error) {
	switch s {
	case "asc":
		return orderAsc, nil
	case "desc":
...
// sortScopeRows sorts rows in-place by slug — which, because slugs carry
...
// first).
func sortScopeRows(rows []scopeRow, order scopesListOrder) {
	if order == orderAsc {
...
//
// Match semantics: case-insensitive literal substring against the scope
// body — no regex, no word boundary. A scope matches if its body contains
...
func applyOverflowNarrow(rows []scopeRow, keywords []string, staxDir string, threshold int) []scopeRow {
	if len(rows) <= threshold || len(keywords) == 0 {
		return rows
	}
	matched := make([]scopeRow, 0, len(rows))
	for _, r := range rows {
		body, ok := readScopeBody(filepath.Join(staxDir, r.slug+scopeFileExt))
		if !ok {
			continue
		}
...
// readScopeBody returns the post-frontmatter body of the scope file at
// path. (false) on missing/unreadable/malformed-frontmatter files so the
// caller can skip them; lintScopeFile surfaces those as per-file findings
// on its own pass.
func readScopeBody(path string) (string, bool) {
...
// which is also what scope frontmatter `systems:` arrays carry).
...
// runScopesList so the filter chain is unit-testable on its own — without
// this helper the only path through status+system filtering was via the
// e2e harness.
func filterPlanRows(rows []scopeRow, statusSet, systemSet map[string]bool) []scopeRow {
	filtered := make([]scopeRow, 0, len(rows))
	for _, r := range rows {
...
// scopeRow is one parsed scope file ready for emission.
...
var (
	scopeStatusRe  = regexp.MustCompile(`(?m)^status:\s*(\S+)\s*$`)
	scopeSystemsRe = regexp.MustCompile(`(?m)^systems:\s*\[([^\]]*)\]\s*$`)
)

// listScopes walks staxDir, parses every file whose name matches
...
//
// Missing staxDir is treated as "no scopes" (returns nil, nil) so callers
// don't need to special-case the pre-init state.
func listScopes(staxDir string, width int, warnW io.Writer) ([]scopeRow, error) {
	entries, err := os.ReadDir(staxDir)
	if err != nil {
	...
	// `<width digits>-<at least one char><scopeFileExt>`. The trailing
	// extension is part of the contract enforced by `scopes lint`; we match
	// the same pattern so stray docs (README.md, _data_systems.yaml) are
	// silently ignored.
	nameRe := regexp.MustCompile(fmt.Sprintf(`^\d{%d}-.+%s$`, width, regexp.QuoteMeta(scopeFileExt)))
	...
	for _, name := range names {
		row, ok := parseScopeFile(filepath.Join(staxDir, name), warnW)
		if !ok {
			continue
		}
...
// parseScopeFile reads one scope file and extracts (slug, status, systems).
...
// so a single bad file never aborts the whole `scopes list` walk.
func parseScopeFile(path string, warnW io.Writer) (scopeRow, bool) {
	if !isSafeScopePath(path) {
	...
	statusMatch := scopeStatusRe.FindStringSubmatch(fm)
	if statusMatch == nil {
		_, _ = fmt.Fprintf(warnW, "warning: %s missing required `status:` field; skipping\n", path)
		return scopeRow{}, false
	}
	systemsMatch := scopeSystemsRe.FindStringSubmatch(fm)
	if systemsMatch == nil {
	...
	slug := strings.TrimSuffix(filepath.Base(path), scopeFileExt)
	return scopeRow{slug: slug, status: statusMatch[1], systems: systems}, true
}
...
// Allowed scope statuses. allowedStatusesSorted is the alphabetised form
...
// the listScopes path (scopeStatusRe / scopeSystemsRe) so both subcommands
// agree on structure.
var (
	...
	// scopeSlugRe matches the post-prefix portion of a scope filename:
	...
	scopeSlugRe = regexp.MustCompile(`^\d+-[a-z0-9-]+$`)
)

// isSafeScopePath rejects any scope-file path that contains a `..`
...
func isSafeScopePath(planPath string) bool {
...
// loadMaxPlanLines mirrors loadPrefixWidth for the max_scope_lines key in
// _config.lock. Falls back to defaultMaxScopeLines on any failure.
func loadMaxPlanLines(staxDir string) int {
	...
	if err != nil {
		return defaultMaxScopeLines
	}
	var cfg struct {
		MaxScopeLines int `json:"max_scope_lines"`
	}
	if err := json.Unmarshal(data, &cfg); err != nil || cfg.MaxScopeLines <= 0 {
		return defaultMaxScopeLines
	}
	return cfg.MaxScopeLines
}

// runScopesLint validates every *.md file in staxDir against the scope schema.
...
//   - Missing staxDir → 0 scopes, exit 0.
//   - Exit 0 if every file passed, exit 1 if any failed.
func runScopesLint(args []string) {
	os.Exit(planLint(args, staxDir, os.Stdout, os.Stderr))
}

// planLint is the testable body of runScopesLint. Exit-code contract:
...
func planLint(args []string, staxDir string, stdout, stderr io.Writer) int {
	fs := flag.NewFlagSet("scopes lint", flag.ContinueOnError)
	fs.SetOutput(stderr)
	cwdFlag := fs.String("cwd", "", "change to this directory before running (like git -C)")
	fs.Usage = func() {
		_, _ = fmt.Fprintln(stderr, "Usage: stax scopes lint [--cwd PATH]")
	}
	if err := fs.Parse(args); err != nil {
		return 2
	}
	if fs.NArg() > 0 {
		_, _ = fmt.Fprintf(stderr, "stax scopes lint takes no arguments (got %q)\n", fs.Arg(0))
		return 2
	}
	if err := applyCwd(*cwdFlag); err != nil {
		_, _ = fmt.Fprintln(stderr, "error:", err)
	...
	for _, f := range files {
		knownSlugs[strings.TrimSuffix(filepath.Base(f), scopeFileExt)] = true
	}
	relations := scanPlansRelations(files)

	okCount, failCount := 0, 0
	for _, path := range files {
		findings := lintScopeFile(path, width, maxLines, reg, knownSlugs, relations, registryPath)
		if len(findings) > 0 {
			failCount++
...
//
//	byID   — "is this id from a scope's frontmatter `systems:` array
...
// the broken scope referencing such a slug will surface its own lint
// finding instead.
...
// lintScopeFile runs every per-file lint check and returns a list of
...
func lintScopeFile(path string, width, maxLines int, reg registry, knownSlugs map[string]bool, relations scopesRelations, registryPath string) []string {
...
// lintFilename returns at most one finding for a non-conforming scope-file
// name. Pulled out so lintScopeFile's main flow reads as a sequence of
// per-section checks.
func lintFilename(name string, width int) []string {
	re := regexp.MustCompile(fmt.Sprintf(`^\d{%d}-[a-z0-9][a-z0-9-]*%s$`, width, regexp.QuoteMeta(scopeFileExt)))
	if re.MatchString(name) {
		return nil
	}
	return []string{fmt.Sprintf("filename %q does not match <prefix>-<slug>%s (prefix width %d, kebab-case slug)", name, scopeFileExt, width)}
}

// lintLineCount enforces the project line cap on a scope file. The +1
...
func lintStatus(fm string) []string {
	m := scopeStatusRe.FindStringSubmatch(fm)
	if m == nil {
...
func lintSystems(fm string, reg registry, registryPath string) (declared, findings []string) {
	m := scopeSystemsRe.FindStringSubmatch(fm)
	if m == nil {
...
// scope, and self-references are rejected. Field name is passed in so the
...
func lintRelationArray(selfSlug, fm, field string, re *regexp.Regexp, knownSlugs map[string]bool) []string {
	...
	for _, slug := range parseInlineSystems(m[1]) {
		if slug == selfSlug {
			findings = append(findings, fmt.Sprintf("%s cannot reference the scope itself", field))
			continue
		}
		if !knownSlugs[slug] {
			findings = append(findings, fmt.Sprintf("%s %q does not match any scope file in the same directory", field, slug))
		}
	}
	return findings
}
...
// symmetric across every scope. For each slug X in `fwd[self]`, X's
...
func lintBidirectional(self string, fwd, back map[string]map[string]bool, fwdField, backField string) []string {
	...
	for x := range fwd[self] {
		if !back[x][self] {
			findings = append(findings, fmt.Sprintf("%s %q but %q does not list this scope in its `%s:` array", fwdField, x, x, backField))
		}
	}
	for y := range back[self] {
		if !fwd[y][self] {
			findings = append(findings, fmt.Sprintf("%s %q but %q does not list this scope in its `%s:` array", backField, y, y, fwdField))
		}
	}
...
// scopesRelations bundles the four cross-scope adjacency maps the linter
// consults for forward/back-link integrity. Each entry is `slug → set of
// slugs in that scope's <field>:` array. Zero values are usable — nil-map
// lookups return false, which is the correct "no link" answer.
type scopesRelations struct {
...
// scanPlansRelations walks every scope file once and populates the four
// adjacency maps in scopesRelations. Files that can't be read or have
// malformed frontmatter contribute nothing — lintScopeFile surfaces those
// as per-file findings on its own pass.
func scanPlansRelations(files []string) scopesRelations {
	r := scopesRelations{
...
//
// declared carries the kebab ids parsed from the scope's frontmatter; EARS
...
// slugify converts a scope title into the kebab-case slug used as the
...
// semantics for the other scope subcommands.
func extractCwdFromHead(args []string, stderr io.Writer) (cwd string, rest []string, ok bool) {
	...
	for len(rest) > 0 {
		...
		case rest[0] == "--cwd":
			if len(rest) < 2 {
				_, _ = fmt.Fprintln(stderr, "stax scopes slugify: --cwd requires a value")
				return "", nil, false
			}
			cwd = rest[1]
			rest = rest[2:]
...
// runScopesSlugify takes a single positional argument (the title) and prints
...
// runScopesSlugify is the only subcommand that takes a single positional.
...
func runScopesSlugify(args []string) {
	os.Exit(planSlugify(args, os.Stdout, os.Stderr))
}

// planSlugify is the testable body of runScopesSlugify. Exit-code
...
func planSlugify(args []string, stdout, stderr io.Writer) int {
	...
	if len(args) >= 1 {
		switch args[0] {
		case "-h", "--help":
			_, _ = fmt.Fprintln(stderr, `Usage: stax scopes slugify [--cwd PATH] "<title>"`)
			return 0
		case "--":
			args = args[1:]
		}
	...
	if len(args) != 1 {
		_, _ = fmt.Fprintln(stderr, `Usage: stax scopes slugify [--cwd PATH] "<title>"`)
		_, _ = fmt.Fprintln(stderr, `stax scopes slugify takes exactly one positional argument: the title (quote it)`)
		return 2
	}
	title := args[0]
	slug := slugify(title)
	if slug == "" {
		_, _ = fmt.Fprintf(stderr, "stax scopes slugify: title %q has no slug-able characters\n", title)
		return 2
	}
