...
// the system's id and display name plus every work item whose frontmatter
...
// rendered markdown heading to <h6>. Work items contain `## Goal`,
// `## Approach`, `## Tasks` section headers and the surrounding page
// chrome (chip, breadcrumb, page title) already supplies the visual
// hierarchy — rendering work-item headings as raw <h2> would dominate the
...
// markdownRenderer is the shared goldmark instance used to render work-item
...
// cycles for no benefit. GFM is enabled so work-item tasks render as
// checkbox-styled list items; raw HTML in the markdown is escaped (no
// WithUnsafe) because work-item files come from a user-editable tree. The
// forceH6Headings transformer pins every heading level to <h6> so work-item
...
//     work item whose frontmatter `systems:` array contains the id, with
//     each work item's markdown body pre-rendered to HTML. An unknown id
...
//     from "system exists but has no work items yet").
//
...
// translates that into a 404. work items is empty (not nil) when the id is
// known but no work item declares it, so the JSON encodes an explicit `[]`.
//
...
func readSystemDetail(staxDir, id string) (systemDetailResponse, bool) {
	...
	for _, f := range files {
		...
		workItems = append(workItems, detail)
	}
	sort.Slice(workItems, func(i, j int) bool {
		if workItems[i].Created != workItems[j].Created {
			return workItems[i].Created > workItems[j].Created
		}
		return workItems[i].Slug > workItems[j].Slug
	})
...
// readWorkItemTitleAndCreated extracts the `title:` and `created:`
// frontmatter values from a work-item file. Missing or unreadable files
// yield empty strings — the caller has already validated through
// parseWorkItem that the file is structurally sound, so this is purely an
// optional-fields read. Frontmatter parsing duplicates the open/close
// fence walk from parseWorkItem because parseWorkItem does not surface these
// two fields.
func readWorkItemTitleAndCreated(planPath string) (title, created string) {
	...
	fm := content[3 : 3+end]
	if m := workItemTitleRe.FindStringSubmatch(fm); m != nil {
		title = strings.Trim(strings.TrimSpace(m[1]), `"'`)
	}
	if m := workItemCreatedRe.FindStringSubmatch(fm); m != nil {
...
//
// Work items whose frontmatter is malformed (missing status, missing
// systems, no closing fence) are skipped silently — parseWorkItem logs to
...
// `stax work-items next-prefix` workflow guarantees for fresh work items but not
...
//     declared system id, OR the work-item body (case-insensitive)
...
func scopeMatchesQuery(staxDir string, s *workItemListItem, needle string) bool {
	...
	body, ok := readWorkItemBody(filepath.Join(staxDir, s.Slug+workItemFileExt))
	if !ok {
		return false
	}
...
// readWorkItemRelations extracts the inline `supersedes:` and
// `superseded_by:` arrays from a work item's frontmatter, then resolves
// each entry to a {slug, title} pair by reading the linked work item's
// own frontmatter. Both relations are optional — a work item that's
...
// reading the linked work item's frontmatter title. Slugs that don't match
// workItemSlugRe are rejected up front so a malformed frontmatter array
...
func enrichRelationSlugs(staxDir string, slugs []string) []workItemRelation {
	...
	for _, slug := range slugs {
		...
		title, _ := readWorkItemTitleAndCreated(filepath.Join(staxDir, slug+workItemFileExt))
		if title == "" {
			title = slug
		}
...
// countScopesPerSystem walks every work-item file in staxDir and tallies
// how many work items declare each system in their frontmatter `systems:`
// array. Work items that fail to parse contribute nothing — same warn-and-
// skip policy parseWorkItem applies elsewhere; per-row tally errors should
