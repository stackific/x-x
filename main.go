...
func main() {
	...
	// ran `stax`, `stax init`, `stax work-items lint`, or anything else. The
	...
	if len(os.Args) >= 2 && !strings.HasPrefix(os.Args[1], "-") {
		...
		case "skills":
			...
			return
		case "work-items":
			// `stax work-items <subcmd>` — work-item-tooling commands (today: next-prefix).
			// Lives in work_items.go.
...
func printAbout() {
	...
	fmt.Println("  stax work-items next-prefix          Print the next unused zero-padded work-item prefix")
	fmt.Println("  stax work-items list                 List work items with slug, status, and declared systems")
	fmt.Println("  stax work-items lint                 Validate every work-item file against the project schema")
	fmt.Println("  stax work-items slugify \"<title>\"    Print the kebab-case slug for a work-item title")
