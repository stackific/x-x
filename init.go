...
func runInit(args []string) {
	...
	// --prefix-width / --max-scope-lines / --review-per are the
	// non-interactive twins of the three scope-tooling prompts. Pass them
	// (alongside --agents and --scope) to drive `stax init` end-to-end
	// without touching the wizard or line prompts.
	prefixWidthFlag := flags.Int("prefix-width", 0, "zero-padded width for scope prefix (positive integer; default seeds the project default)")
	maxScopeLinesFlag := flags.Int("max-scope-lines", 0, "line-count ceiling enforced by `stax scopes lint` (positive integer; default seeds the project default)")
	reviewPerFlag := flags.String("review-per", "", "task|scope — pause for review after every task or every scope")
	...
	flags.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "             [--prefix-width N] [--max-scope-lines N] [--review-per task|scope]")
	...
	// otherwise let `--prefix-width=-1`, `--max-scope-lines=0`, `--agents=`,
	...
	// to check.
	validateInitFlagsOrExit(flags, prefixWidthFlag, maxScopeLinesFlag, &agentsFlag, reviewPerFlag)

	// Honor --cwd before any cwd-dependent call — os.Getwd, checkProject,
	// scopeRootFor(scopeProject, cwd), and writeScopesScaffold all expect
	...
	// which is what writeScopesScaffold's writeIfAbsent semantics rely on.
	...
	cfg, err := resolveInitConfig(initFlags{
		...
		maxScopeLines: *maxScopeLinesFlag,
	...
	if err := writeScopesScaffold(cwd, cfg); err != nil {
		fmt.Fprintf(os.Stderr, "warning: %v\n", err)
	}

	fmt.Println("\nDone.")
	// Scope files are first-class repo content (frontmatter + EARS tasks),
	// not local state. Nudge the user to commit them so the team shares
	// the same scope history. Phrased as a tip rather than auto-editing
	// .gitignore so we never touch git config behind the user's back.
	fmt.Printf("\nTip: commit %s/ to git so your team shares scope history.\n", staxDir)
...
func validateInitFlagsOrExit(flags *flag.FlagSet, prefixWidth, maxScopeLines *int, agents *stringSliceFlag, reviewPer *string) {
	if err := validateInitFlags(flags, prefixWidth, maxScopeLines, agents, reviewPer); err != nil {
		exitErr(err)
	}
}
...
func validateInitFlags(flags *flag.FlagSet, prefixWidth, maxScopeLines *int, agents *stringSliceFlag, reviewPer *string) error {
	var firstErr error
	flags.Visit(func(fl *flag.Flag) {
		...
		case "max-scope-lines":
			if *maxScopeLines <= 0 {
				firstErr = fmt.Errorf("--max-scope-lines must be positive, got %d", *maxScopeLines)
			}
		case "agents":
			if len(*agents) == 0 {
		...
		case "review-per":
			if *reviewPer == "" {
				firstErr = fmt.Errorf("invalid --review-per: %q (expected %s or %s)", *reviewPer, reviewPerTask, reviewPerScope)
			}
		}
	})
	return firstErr
...
func (f initFlags) toConfig() (initConfig, error) {
	...
	if f.maxScopeLines <= 0 {
		return initConfig{}, fmt.Errorf("--max-scope-lines must be positive, got %d", f.maxScopeLines)
	}
	return initConfig{
		agents:        agents,
		scope:         scope,
		prefixWidth:   f.prefixWidth,
		maxScopeLines: f.maxScopeLines,
		reviewPer:     review,
	}, nil
}
...
func runLinePrompts(f initFlags, in io.Reader) (initConfig, error) {
	...
	if cfg.maxScopeLines <= 0 {
		cfg.maxScopeLines, err = promptMaxScopeLines(r)
		if err != nil {
			return initConfig{}, err
		}
	}
...
func runHuhWizard(f initFlags) (initConfig, error) {
	...
	maxPlanLinesStr := strconv.Itoa(maxScopeLines)
	...
	form := huh.NewForm(
		...
		huh.NewGroup(
			huh.NewInput().
				Title("Prefix width for scope file").
				Description("Zero-padded width for scope filename (e.g. width 4 → 0001-foo.md). Higher values give more headroom before scope number run out.").
				Value(&prefixWidthStr).
				Validate(validatePositiveInt),
		),
		huh.NewGroup(
			huh.NewInput().
				Title("Maximum lines per scope").
				Description("Keeps AI agents on a short leash: forces them to split sprawling work into smaller, reviewable scopes.").
		...
		huh.NewGroup(
			huh.NewSelect[string]().
				...
				Description("`task` — review each EARS criterion as the planner finishes it (tight loop, more interruptions).  `scope` — review at the end of each scope (looser loop, larger diffs).").
				Options(
					huh.NewOption(reviewPerTask+" — tight feedback loop", reviewPerTask),
					huh.NewOption(reviewPerScope+" — review only at scope boundaries", reviewPerScope),
				).
				Value(&reviewPer),
		),
	)
	...
	if err != nil || ml <= 0 {
		return initConfig{}, fmt.Errorf("invalid max-scope-lines from wizard: %q", maxPlanLinesStr)
	}
...
// prefix-width and max-scope-lines fields. Strings only — caller parses
...
// writeScopesScaffold creates the project-local .stax/ directory and seeds
// the two files that the scope tooling expects to find on disk:
//
//	_data_systems.yaml — empty placeholder; populated by the user as systems are added
//	_config.lock  — scope-tooling pins (prefix_width, max_scope_lines, review_per)
//
...
// project defaults.
func writeScopesScaffold(cwd string, cfg initConfig) error {
	dir := filepath.Join(cwd, staxDir)
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return fmt.Errorf("create %s: %w", dir, err)
	}
	// Empty placeholder — the scope tooling populates this as the project
	...
	lock := struct {
		...
		MaxScopeLines: cfg.maxScopeLines,
...
// write the given content with 0o600 perms. Used by writeScopesScaffold.
...
func parseReviewPer(s string) (string, error) {
	switch s {
	case reviewPerTask, reviewPerScope:
		return s, nil
	default:
		return "", fmt.Errorf("invalid --review-per: %q (expected %s or %s)",
			s, reviewPerTask, reviewPerScope)
	}
}
...
// zero-padded scope-prefix width. Empty input (blank line / EOF before
...
func promptPrefixWidth(in io.Reader) (int, error) {
	fmt.Println("Prefix width for scope file")
	fmt.Println("  Zero-padded width for scope filename (e.g. width 4 → 0001-foo.md).")
	fmt.Println("  Higher = more headroom before scope number run out.")
	fmt.Printf("Choose [default %d]: ", defaultPrefixWidth)
	return readPositiveIntLine(in, defaultPrefixWidth, "prefix-width")
}

// promptMaxScopeLines reads one line from `in` and parses it as the scope
// line-count cap. Same default-on-empty semantics as promptPrefixWidth.
// The cap is what `stax scopes lint` enforces — tight values keep AI agents
// from sprawling, looser values let larger scopes breathe.
func promptMaxScopeLines(in io.Reader) (int, error) {
	fmt.Println("Maximum lines per scope")
	fmt.Println("  Keeps AI agents on a short leash:")
	fmt.Println("  forces them to split sprawling work into smaller, reviewable scopes.")
	fmt.Printf("Choose [default %d]: ", defaultMaxScopeLines)
	return readPositiveIntLine(in, defaultMaxScopeLines, "max-scope-lines")
}

// promptReviewPer reads one line from `in` and parses it as the
// review cadence: "1" → task, "2" → scope. Empty input accepts the
...
func promptReviewPer(in io.Reader) (string, error) {
	...
	fmt.Printf("  2) %s — review only at scope boundaries (looser loop, larger diffs)\n", reviewPerScope)
...
// promptMaxScopeLines: read one line, trim, accept default on empty, parse
...
// the relative staxDir each scope subcommand passes through, and the
...
// exitErr (init / skills) or their own stderr+exit-2 pattern (scopes).
...
//
//	staxDir/staxLockFile (the scope-tooling lock pin)
//
// Missing → not an initialized project. Other files under staxDir
// (the systems registry, scope file) are not required by the check.
...
// fresh init without losing scopes or the systems registry. The function
...
// (`scopes *`, `skills remove --project`) calls before doing real work.
