...
const (
	...
	//   - Project scope: <cwd>/<staxDir>/ holds the scope-tooling scaffold
	//     (staxLockFile, staxSystemsFile, *.md scope file).
	...
	// staxLockFile pins the scope-tooling defaults inside the project-scope
	...
	// scopeFileExt is the on-disk extension every scope file carries. Pulled
	// out as a constant so scope.go's filename-format regexes, glob, and
	// suffix-trimming all reference the same value — adding a future
	// alternative extension (e.g. ".scope.md") is then a one-line change.
	scopeFileExt = ".md"
)
...
// Scope-tooling defaults pinned into <staxDir>/<staxLockFile> by
// writeScopesScaffold during `stax init`. The `stax scopes` subcommands read
...
const (
	// defaultPrefixWidth is the zero-padded width of scope-file numeric
	// prefixes (e.g. width 4 → "0001-foo.md"). Bump to widen prefixes.
	defaultPrefixWidth = 4

	// defaultMaxScopeLines is the line-count ceiling `stax scopes lint`
	// enforces on a single scope file (frontmatter + body, inclusive).
	defaultMaxScopeLines = 30

	// scopesListOverflowThreshold is the row count above which
	// `stax scopes list` activates the optional `--overflow-keywords` narrow.
	// At or below this count every matching scope is returned regardless
	...
	scopesListOverflowThreshold = 20

	// defaultReviewPer controls whether the planner pauses for review
	// after every "task" or after every "scope" (other valid value).
	defaultReviewPer = reviewPerTask

	// reviewPerTask / reviewPerScope are the two valid values for
...
var ownedFiles = []string{
	// Scope-tooling scaffold seeded by writeScopesScaffold (project scope only).
...
//
// `scope` is the project's single name for `.stax/<prefix>-<slug>.md`
// files across every surface: the `/scope` bundled skill that authors
// them, the `stax scopes *` CLI subcommands that manage them, the HTTP
// API that serves them, and the web UI that renders them.
const (
