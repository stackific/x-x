...
func hookFixtureJSON(t *testing.T) (bundle, user string) {
  ...
  "hooks": {
    "PostToolUse": [
      {"matcher": "Write|Edit", "hooks": [{"type": "command", "command": "stax work-items lint"}]}
    ]
  }
}
`
...
func TestSubtractHooks_PreservesUserTweakedVariant(t *testing.T) {
	bundle := decodeJSON(t, `{"hooks":{"PostToolUse":[{"matcher":"Write|Edit","hooks":[{"type":"command","command":"stax work-items lint"}]}]}}`).(map[string]any)[configHooksKey]
	user := decodeJSON(t, `{"hooks":{"PostToolUse":[{"matcher":"Write|Edit","hooks":[{"type":"command","command":"stax work-items lint --verbose"}]}]}}`).(map[string]any)[configHooksKey]
...
func TestSubtractHooks_BundledEventKeyMissingInUser(t *testing.T) {
	bundle := decodeJSON(t, `{"hooks":{"Stop":[{"matcher":"","hooks":[{"type":"command","command":"stax work-items lint"}]}]}}`).(map[string]any)[configHooksKey]
