...
"""End-to-end: drive omp through scope-supersede + artifact-replace.

Mirror of test_claude_reminders_supersedes_todo.py against the omp driver.

Sequence:
  1. /skill:scope a todo list app.
  2. /skill:scope a reminders app that SUPERSEDES the todo scope.
  3. /skill:stax — executes the queue; per agents/skills/ship/SKILL.md step
     3.4, when the executor finishes the successor scope it flips the
...
def test_omp_reminders_supersedes_todo(
  ...
  assert todo_scope.frontmatter.get("status") == "superseded", (
    f"todo scope should be status=superseded after /skill:stax ran the "
