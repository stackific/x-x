...
"""End-to-end: drive Kilo Code through plan-supersede + artifact-replace.

Mirror of test_opencode_reminders_supersedes_todo.py adapted for the Kilo
Code headless path. Each skill invocation is `kilo run --command <name>
<task>`; Kilo resolves the SKILL.md by frontmatter `name:` (same
mechanic as opencode, from which Kilo is forked).
...
def test_kilocode_reminders_supersedes_todo(
  ...
  assert todo_run.exit_code == 0, (
    f"kilo exited {todo_run.exit_code} during /x-plan todo; "
  ...
  assert reminders_run.exit_code == 0, (
    f"kilo exited {reminders_run.exit_code} during /x-plan reminders; "
