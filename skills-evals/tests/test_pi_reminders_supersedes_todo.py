...
"""End-to-end: drive Pi through scope-supersede + artifact-replace.

Mirror of test_opencode_reminders_supersedes_todo.py adapted for the Pi
headless path. Each skill invocation is `pi --mode json -p "/skill:<name>
<task>"`; Pi resolves the SKILL.md by frontmatter `name:`.
...
Two distinct assertion classes:
  ...
  Artifact correctness (DeepEval GEval via ArtifactJudge):
    - the final workspace contains a working REMINDERS app, not a todo
      list — even if the executor ran both scopes in order, the
...
def test_pi_reminders_supersedes_todo(
  ...
  assert todo_run.exit_code == 0, (
    f"pi exited {todo_run.exit_code} during /scope todo; "
  ...
  assert reminders_run.exit_code == 0, (
    f"pi exited {reminders_run.exit_code} during /scope reminders; "
