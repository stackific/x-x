...
"""End-to-end: drive Kilo Code through two x-plan calls that exercise
the extends mechanic.

Mirror of test_opencode_plan_extends.py adapted for the Kilo Code
headless path. Each x-plan invocation is `kilo run --command x-plan
<task>`; Kilo's command resolver (inherited from opencode) finds the
SKILL.md by frontmatter `name:`.
...
def test_kilocode_plan_extends(workspace: Path, tmp_path: Path) -> None:
