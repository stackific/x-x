...
"""End-to-end: drive Codex through /x-plan + /x-x for a TODO app task.

Mirror of test_claude_todo.py against the Codex driver. The TASK string
and the assertions are agent-agnostic — they exercise the skills' shipped
behavior. Only the driver import changes.
"""
...
def test_codex_builds_todo_app(workspace: Path, tmp_path: Path) -> None:
  ...
  assert plan_run.exit_code == 0, (
    f"codex exited {plan_run.exit_code} during /x-plan; "
