...
"""End-to-end: drive Kilo through /x-plan + /x-x for a TODO app task.

Flow per the user's spec:
  1. The "user" (this test) invokes /x-plan with the TODO task. Auto-reply
     'yes' (via kilo --continue / --session) until /x-plan stops asking.
...
def test_kilo_builds_todo_app(workspace: Path, tmp_path: Path) -> None:
