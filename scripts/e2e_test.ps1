...
Set-Variable -Option Constant -Name SCOPES_LIST_OVERFLOW_THRESHOLD   -Value 20
...
# Seeds a project scaffold the way `stax init --scope project` would: scopeDir
...
function Initialize-ProjectScaffold {
  ...
  $scopeDir = Join-Path $Path $STAX_DIR
  New-Item -ItemType Directory -Force -Path $scopeDir | Out-Null
  New-Item -ItemType File -Force -Path (Join-Path $scopeDir $STAX_SYSTEMS_FILE) | Out-Null
  $lockJson = @"
{
...
"@
  Set-Content -LiteralPath (Join-Path $scopeDir $STAX_LOCK_FILE) -Value $lockJson -Encoding ascii
}

# Zero-padded prefix helper — mirrors the bash `prefix WIDTH N`. Used by every
# scope-file fixture in the scopes-list/scopes-lint sections.
...
# Writes a minimally-valid scope file (frontmatter + body) for use by tests
# that exercise `scopes list`. Body is the third positional so cases can
# override per test.
...
Assert-Contains 'scopes next-prefix listed'     $RunOut 'stax scopes next-prefix'
Assert-Contains 'scopes list listed'            $RunOut 'stax scopes list'
Assert-Contains 'scopes lint listed'            $RunOut 'stax scopes lint'
Assert-Contains 'scopes slugify listed'         $RunOut 'stax scopes slugify'
...
# ---------- scopes subcommand routing ----------

Start-Case 'stax scopes (no subcommand) prints usage to stderr and exits 2'
Invoke-XX scopes
Assert-Eq       'exit 2'       $RunRC 2
Assert-Contains 'usage header' $RunErr 'Usage: stax scopes <subcommand>'
...
Start-Case 'stax scopes <typo> exits 2 with diagnostic'
Invoke-XX scopes frobnicate
Assert-Eq       'exit 2'     $RunRC 2
Assert-Contains 'diagnostic' $RunErr 'unknown scopes subcommand: frobnicate'

# ---------- scopes slugify (no project-marker check) ----------

Start-Case 'scopes slugify lowercases ASCII'
Invoke-XX scopes slugify 'Add Payment Retry'
Assert-Eq 'exit 0'      $RunRC 0
Assert-Eq 'slug printed' $RunOut 'add-payment-retry'

Start-Case 'scopes slugify collapses runs of non-alnum'
Invoke-XX scopes slugify 'foo!!!bar---baz'
Assert-Eq 'exit 0'      $RunRC 0
Assert-Eq 'slug printed' $RunOut 'foo-bar-baz'

Start-Case 'scopes slugify trims leading/trailing dashes'
Invoke-XX scopes slugify '---foo---'
Assert-Eq 'exit 0'       $RunRC 0
Assert-Eq 'slug printed' $RunOut 'foo'

Start-Case 'scopes slugify accepts a double-dash-prefixed title WITHOUT --'
# runScopesSlugify bypasses flag.Parse so `--draft note` is treated as the
# title verbatim, not as a flag. Without that fix, flag.Parse would reject
# this with "flag provided but not defined: -draft".
Invoke-XX scopes slugify '--draft note'
Assert-Eq 'exit 0'         $RunRC 0
Assert-Eq 'double-dash slug' $RunOut 'draft-note'

Start-Case 'scopes slugify accepts pure numerics'
Invoke-XX scopes slugify '12345'
Assert-Eq 'exit 0'       $RunRC 0
Assert-Eq 'numeric slug' $RunOut '12345'

Start-Case 'scopes slugify rejects missing argument'
Invoke-XX scopes slugify
Assert-Eq       'exit 2'     $RunRC 2
Assert-Contains 'diagnostic' $RunErr 'takes exactly one positional argument'

Start-Case 'scopes slugify rejects multiple positional arguments'
Invoke-XX scopes slugify foo bar
Assert-Eq       'exit 2'     $RunRC 2
Assert-Contains 'diagnostic' $RunErr 'takes exactly one positional argument'

Start-Case 'scopes slugify rejects an unsluggable title'
Invoke-XX scopes slugify '!!! ??? ###'
Assert-Eq       'exit 2'     $RunRC 2
Assert-Contains 'diagnostic' $RunErr 'no slug-able characters'

Start-Case 'scopes slugify works outside a stax project'
$noProject = New-FreshProject
Push-Location $noProject
try {
  Invoke-XX scopes slugify 'Some Title'
  Assert-Eq 'exit 0'       $RunRC 0
...
Start-Case 'scopes next-prefix in non-project exits 2 with diagnostic'
$noProj = New-FreshProject
Push-Location $noProj
try {
  Invoke-XX scopes next-prefix
  Assert-Eq       'exit 2'     $RunRC 2
  Assert-Contains 'diagnostic' $RunErr 'not a stax project'
} finally { Pop-Location }

Start-Case 'scopes list in non-project exits 2 with diagnostic'
Push-Location $noProj
try {
  Invoke-XX scopes list
  Assert-Eq       'exit 2'     $RunRC 2
  Assert-Contains 'diagnostic' $RunErr 'not a stax project'
} finally { Pop-Location }

Start-Case 'scopes lint in non-project exits 2 with diagnostic'
Push-Location $noProj
try {
  Invoke-XX scopes lint
  Assert-Eq       'exit 2'     $RunRC 2
  Assert-Contains 'diagnostic' $RunErr 'not a stax project'
} finally { Pop-Location }
...
Start-Case 'scopes next-prefix returns the zero-padded first prefix on a fresh project'
...
Start-Case 'scopes next-prefix walks past existing scopes'
$projB = New-FreshProject
Initialize-ProjectScaffold $projB
$staxDir = Join-Path $projB $STAX_DIR
Write-Scope $staxDir '0003-charlie.md' 'valid' 'auth-service'
Write-Scope $staxDir '0005-echo.md'    'valid' 'auth-service'
Write-Scope $staxDir '0002-bravo.md'   'valid' 'auth-service'
Push-Location $projB
try {
  Invoke-XX scopes next-prefix
  Assert-Eq 'exit 0'           $RunRC 0
...
Start-Case 'scopes next-prefix honors prefix_width pin from _config.lock'
$projC = New-FreshProject
Initialize-ProjectScaffold $projC
$lockC = Join-Path (Join-Path $projC $STAX_DIR) $STAX_LOCK_FILE
Set-Content -LiteralPath $lockC -Value '{"prefix_width":6,"max_scope_lines":30,"review_per":"task"}' -Encoding ascii
Push-Location $projC
try {
  Invoke-XX scopes next-prefix
  Assert-Eq 'exit 0'             $RunRC 0
...
Start-Case 'scopes next-prefix rejects positional arguments'
Push-Location $proj
try {
  Invoke-XX scopes next-prefix extra
  Assert-Eq       'exit 2'     $RunRC 2
...
Start-Case 'scopes list emits tab-separated rows sorted by prefix descending (default)'
$projL = New-FreshProject
Initialize-ProjectScaffold $projL
$staxDirL = Join-Path $projL $STAX_DIR
Write-Scope $staxDirL '0002-bravo.md'   'deprecated' 'billing'
Write-Scope $staxDirL '0001-alpha.md'   'valid'      'auth,billing'
Write-Scope $staxDirL '0003-charlie.md' 'superseded' 'auth'
Push-Location $projL
try {
  Invoke-XX scopes list
  Assert-Eq 'exit 0' $RunRC 0
...
Start-Case 'scopes list --order=asc reverses to prefix-ascending'
Push-Location $projL
try {
  Invoke-XX scopes list --order=asc
  Assert-Eq 'exit 0' $RunRC 0
  $expected = @(
    "0001-alpha`tvalid`tauth,billing"
    "0002-bravo`tdeprecated`tbilling"
...
Start-Case 'scopes list --status filter keeps only matching rows'
Push-Location $projL
try {
  Invoke-XX scopes list --status valid
  Assert-Eq 'exit 0'           $RunRC 0
  Assert-Eq 'only valid rows' $RunOut "0001-alpha`tvalid`tauth,billing"
} finally { Pop-Location }

Start-Case 'scopes list --status accepts comma list'
Push-Location $projL
try {
  Invoke-XX scopes list --status 'valid,superseded'
  Assert-Eq 'exit 0' $RunRC 0
  $expected = @(
    "0003-charlie`tsuperseded`tauth"
    "0001-alpha`tvalid`tauth,billing"
...
Start-Case 'scopes list --system filter matches by kebab id'
Push-Location $projL
try {
  Invoke-XX scopes list --system billing
  Assert-Eq 'exit 0' $RunRC 0
  $expected = @(
    "0002-bravo`tdeprecated`tbilling"
    "0001-alpha`tvalid`tauth,billing"
...
Start-Case 'scopes list combined --status and --system intersects both'
Push-Location $projL
try {
  Invoke-XX scopes list --status valid --system auth
  Assert-Eq 'exit 0' $RunRC 0
  Assert-Eq 'status+system intersection' $RunOut "0001-alpha`tvalid`tauth,billing"
} finally { Pop-Location }

Start-Case 'scopes list --system <unknown id> returns zero rows'
Push-Location $projL
try {
  Invoke-XX scopes list --system never-declared
  Assert-Eq 'exit 0'    $RunRC 0
  Assert-Eq 'empty out' $RunOut ''
} finally { Pop-Location }

Start-Case 'scopes list rejects positional arguments'
Push-Location $projL
try {
  Invoke-XX scopes list foo
  Assert-Eq       'exit 2'     $RunRC 2
  Assert-Contains 'diagnostic' $RunErr 'takes no positional'
} finally { Pop-Location }

Start-Case 'scopes list warns on malformed frontmatter but keeps siblings'
$projWarn = New-FreshProject
Initialize-ProjectScaffold $projWarn
$staxDirW = Join-Path $projWarn $STAX_DIR
Write-Scope $staxDirW '0002-ok.md' 'valid' 'auth'
Set-Content -LiteralPath (Join-Path $staxDirW '0001-broken.md') -Value 'not a scope' -Encoding ascii
Push-Location $projWarn
try {
  Invoke-XX scopes list
  Assert-Eq       'exit 0'       $RunRC 0
  Assert-Eq       'kept the OK scope' $RunOut "0002-ok`tvalid`tauth"
  Assert-Contains 'warned about broken' $RunErr '0001-broken.md'
} finally { Pop-Location }

Start-Case 'scopes list ignores files that do not match the <prefix>-<slug>.md pattern'
$projP = New-FreshProject
Initialize-ProjectScaffold $projP
$staxDirP = Join-Path $projP $STAX_DIR
Write-Scope $staxDirP '0001-keep.md' 'valid' 'auth'
...
# ---------- scopes lint (subset; full lint is exercised by the bash e2e) ----------

Start-Case 'scopes lint passes on a happy-path scope'
...
$scope1 = Join-Path $staxDirLN '0001-foo.md'
$body1 = @"
---
title: foo
status: valid
...
"@
Set-Content -LiteralPath $scope1 -Value $body1 -Encoding ascii
...
Start-Case 'scopes lint flags filename slug mismatch with title'
...
$scope2 = Join-Path $staxDirLN2 '0001-foo.md'
$body2 = @"
---
...
- [ ] The Auth Service shall do.
"@
Set-Content -LiteralPath $scope2 -Value $body2 -Encoding ascii
...
Start-Case 'scopes lint flags an unknown declared system'
...
Start-Case 'scopes lint flags a dangling supersedes link'
...
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 4 --max-scope-lines 30 --review-per task
  Assert-Eq       'exit 0'             $RunRC 0
  ...
  Assert-Contains 'lock honors --max-scope-lines=30' $lockContent '"max_scope_lines": 30'
...
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width=-1 --max-scope-lines 30 --review-per task
  Assert-Eq       'exit 1'     $RunRC 1
  Assert-Contains 'diagnostic' $RunErr '--prefix-width must be positive'
} finally { Pop-Location }

Start-Case 'init --max-scope-lines=0 is rejected'
Reset-UserHome
$projBad2 = New-FreshProject
Push-Location $projBad2
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 4 --max-scope-lines 0 --review-per task
  Assert-Eq       'exit 1'     $RunRC 1
  Assert-Contains 'diagnostic' $RunErr '--max-scope-lines must be positive'
} finally { Pop-Location }
...
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 4 --max-scope-lines 30 --review-per commit
  Assert-Eq       'exit 1'     $RunRC 1
...
Start-Case 'init interactive: prompts for agents/scope/prefix-width/max-scope-lines/review-per'
Reset-UserHome
$projInt = New-FreshProject
Push-Location $projInt
try {
  # Order: agents (blank = all defaults to all), scope (1=project),
  # prefix-width (blank = default), max-scope-lines (blank = default),
  # review-per (1=task).
...
try {
  ...
  # with /scope, /ship, and the `stax scopes *` CLI subcommands.
...
Start-Case 'scopes list tolerates CRLF line endings in scope files'
...
# Write scope with explicit CRLF — mimics what Windows editors produce.
...
# name filter. The CLI never produces such names because every scope slug is
...
# (whatever way), `scopes list` ignores them and `scopes next-prefix` keeps
# walking the conforming siblings.

Start-Case 'scopes list ignores reserved-name files at the .stax/ root'
$projRes = New-FreshProject
Initialize-ProjectScaffold $projRes
$staxDirRes = Join-Path $projRes $STAX_DIR
Write-Scope $staxDirRes '0001-foo.md' 'valid' 'auth'
...
if ($createdReserved.Count -eq 0) {
  ...
  try {
    Invoke-XX scopes list
    Assert-Eq 'exit 0' $RunRC 0
    Assert-Eq 'only conforming scope listed' $RunOut "0001-foo`tvalid`tauth"
    Invoke-XX scopes next-prefix
...
Start-Case 'scopes list treats filenames case-insensitively (Windows NTFS default)'
$projCI = New-FreshProject
Initialize-ProjectScaffold $projCI
$staxDirCI = Join-Path $projCI $STAX_DIR
Write-Scope $staxDirCI '0001-foo.md' 'valid' 'auth'
...
try {
  ...
  # Same `scopes list`, lookup-by-uppercase shouldn't duplicate the row.
  Invoke-XX scopes list --system AUTH
...
Start-Case 'scopes next-prefix tolerates a UTF-8 BOM in _config.lock'
...
$bomBytes = [System.Text.Encoding]::UTF8.GetPreamble() +
            [System.Text.Encoding]::UTF8.GetBytes('{"prefix_width":5,"max_scope_lines":30,"review_per":"task"}')
...
# ---------- scopes list: --overflow-keywords + threshold behavior ----------
#
...
# Helper: seed N body-only scopes whose body is exactly the supplied content.
function Write-ScopeWithBody {
  param(
    [Parameter(Mandatory)][string]$StaxDir,
    [Parameter(Mandatory)][string]$Name,
...
function Add-ManyPlans {
  ...
  for ($i = 1; $i -le $Count; $i++) {
    $name = '{0:D4}-scope{1:D3}.md' -f $i, $i
    Write-ScopeWithBody -StaxDir $StaxDir -Name $name -Body "$i $Body"
  }
}

Start-Case 'scopes list --overflow-keywords is a no-op below threshold'
...
try {
  Invoke-XX scopes list --overflow-keywords zzz-never-matches
  Assert-Eq 'exit 0' $RunRC 0
  $rowCount = ($RunOut -split "`n").Count
  Assert-Eq 'all 5 rows returned (no-op below threshold)' $rowCount 5
} finally { Pop-Location }

Start-Case 'scopes list --overflow-keywords engages above threshold, matches body'
$projOK2 = New-FreshProject
Initialize-ProjectScaffold $projOK2
$staxDirOK2 = Join-Path $projOK2 $STAX_DIR
$over = $SCOPES_LIST_OVERFLOW_THRESHOLD + 5
Add-ManyPlans -StaxDir $staxDirOK2 -Count $over -Body 'generic body'
# Replace one body with a keyword the harness will look for.
Write-ScopeWithBody -StaxDir $staxDirOK2 -Name '0007-scope007.md' -Body 'this scope covers exponential retry backoff'
Push-Location $projOK2
try {
  Invoke-XX scopes list --overflow-keywords retry
  Assert-Eq       'exit 0'          $RunRC 0
  Assert-Contains 'matched scope in output' $RunOut 'scope007'
  $rowCount = ($RunOut -split "`n").Count
  Assert-Eq 'exactly the matched row' $rowCount 1
} finally { Pop-Location }

Start-Case 'scopes list --overflow-keywords falls back to top-N when no body matches'
$projOK3 = New-FreshProject
Initialize-ProjectScaffold $projOK3
$staxDirOK3 = Join-Path $projOK3 $STAX_DIR
Add-ManyPlans -StaxDir $staxDirOK3 -Count ($SCOPES_LIST_OVERFLOW_THRESHOLD + 3) -Body 'nothing relevant here'
Push-Location $projOK3
try {
  Invoke-XX scopes list --overflow-keywords zzz-never-matches
  Assert-Eq 'exit 0' $RunRC 0
  $rowCount = ($RunOut -split "`n").Count
  Assert-Eq 'falls back to threshold rows' $rowCount $SCOPES_LIST_OVERFLOW_THRESHOLD
} finally { Pop-Location }

Start-Case 'scopes list --overflow-keywords matches body-only (frontmatter ignored)'
$projOK4 = New-FreshProject
Initialize-ProjectScaffold $projOK4
$staxDirOK4 = Join-Path $projOK4 $STAX_DIR
Add-ManyPlans -StaxDir $staxDirOK4 -Count ($SCOPES_LIST_OVERFLOW_THRESHOLD + 1) -Body 'generic body'
Push-Location $projOK4
try {
  # 'auth' appears in every scope's frontmatter (systems: [auth]) but NEVER in body.
  Invoke-XX scopes list --overflow-keywords auth
  Assert-Eq 'exit 0' $RunRC 0
  $rowCount = ($RunOut -split "`n").Count
  Assert-Eq 'frontmatter-only keyword triggers top-N fallback' $rowCount $SCOPES_LIST_OVERFLOW_THRESHOLD
} finally { Pop-Location }

Start-Case 'scopes list --overflow-keywords is case-insensitive'
$projOK5 = New-FreshProject
Initialize-ProjectScaffold $projOK5
$staxDirOK5 = Join-Path $projOK5 $STAX_DIR
Add-ManyPlans -StaxDir $staxDirOK5 -Count $SCOPES_LIST_OVERFLOW_THRESHOLD -Body 'generic body'
Write-ScopeWithBody -StaxDir $staxDirOK5 -Name "$('{0:D4}' -f ($SCOPES_LIST_OVERFLOW_THRESHOLD + 1))-extra.md" -Body 'Uppercase MATCH inside body'
Push-Location $projOK5
try {
  Invoke-XX scopes list --overflow-keywords match
  Assert-Eq       'exit 0' $RunRC 0
  Assert-Contains 'lowercase keyword matches uppercase body' $RunOut 'extra'
} finally { Pop-Location }

Start-Case 'scopes list --overflow-keywords accepts multiple terms (OR semantics)'
$projOK6 = New-FreshProject
Initialize-ProjectScaffold $projOK6
$staxDirOK6 = Join-Path $projOK6 $STAX_DIR
Add-ManyPlans -StaxDir $staxDirOK6 -Count $SCOPES_LIST_OVERFLOW_THRESHOLD -Body 'generic body'
Write-ScopeWithBody -StaxDir $staxDirOK6 -Name "$('{0:D4}' -f ($SCOPES_LIST_OVERFLOW_THRESHOLD + 1))-alpha.md" -Body 'mentions webhook only'
Write-ScopeWithBody -StaxDir $staxDirOK6 -Name "$('{0:D4}' -f ($SCOPES_LIST_OVERFLOW_THRESHOLD + 2))-bravo.md" -Body 'mentions retry only'
Push-Location $projOK6
try {
  Invoke-XX scopes list --overflow-keywords 'webhook,retry'
  Assert-Eq       'exit 0' $RunRC 0
  Assert-Contains 'OR match: alpha present' $RunOut 'alpha'
  Assert-Contains 'OR match: bravo present' $RunOut 'bravo'
} finally { Pop-Location }

Start-Case 'scopes list --overflow-keywords repeated flag = comma list'
Push-Location $projOK6
try {
  Invoke-XX scopes list --overflow-keywords webhook --overflow-keywords retry
  Assert-Eq       'exit 0' $RunRC 0
  Assert-Contains 'repeated-flag alpha' $RunOut 'alpha'
  Assert-Contains 'repeated-flag bravo' $RunOut 'bravo'
} finally { Pop-Location }

Start-Case 'scopes list combined --system + --overflow-keywords narrows correctly'
$projOK7 = New-FreshProject
Initialize-ProjectScaffold $projOK7
$staxDirOK7 = Join-Path $projOK7 $STAX_DIR
$over7 = $SCOPES_LIST_OVERFLOW_THRESHOLD + 5
...
$scope7Match = '0007-payment007.md'
$scope7MatchBody = "---`nstatus: valid`nsystems: [payment-service]`n---`nthis scope covers exponential retry backoff"
Set-Content -LiteralPath (Join-Path $staxDirOK7 $scope7Match) -Value  -Encoding ascii
# An unrelated-system scope with the same body keyword — must be filtered out by --system.
...
try {
  Invoke-XX scopes list --system payment-service --overflow-keywords retry
  Assert-Eq           'exit 0' $RunRC 0
...
Start-Case 'scopes list --status + --system + --overflow-keywords narrows status∩system > threshold'
...
# Threshold+2 scopes, all status=valid + system=payment-service, body
# WITHOUT the keyword. Two of them (5, 17) get overwritten below with
# bodies that DO contain "retry".
$overSSO = $SCOPES_LIST_OVERFLOW_THRESHOLD + 2
for ($i = 1; $i -le $overSSO; $i++) {
  $nameSSO = '{0:D4}-scope{1:D3}.md' -f $i, $i
...
foreach ($matchN in 5, 17) {
  $matchName = '{0:D4}-scope{1:D3}.md' -f $matchN, $matchN
...
$ssoWrongStatusBody = "---`nstatus: deprecated`nsystems: [payment-service]`n---`ndeprecated scope that mentions retry"
Set-Content -LiteralPath (Join-Path $staxDirSSO '0098-wrong-status.md') -Value $ssoWrongStatusBody -Encoding ascii
$ssoWrongSystemBody = "---`nstatus: valid`nsystems: [other-service]`n---`nother-service scope that mentions retry"
...
try {
  Invoke-XX scopes list --status valid --system payment-service --overflow-keywords retry
  Assert-Eq          'exit 0'                                    $RunRC 0
  Assert-Contains    'scope005 in match'                          $RunOut 'scope005'
  Assert-Contains    'scope017 in match'                          $RunOut 'scope017'
...
Start-Case 'scopes list --order=desc explicit default'
$projOK8 = New-FreshProject
Initialize-ProjectScaffold $projOK8
$staxDirOK8 = Join-Path $projOK8 $STAX_DIR
Write-Scope $staxDirOK8 '0001-alpha.md' 'valid' 'auth'
Write-Scope $staxDirOK8 '0002-bravo.md' 'valid' 'auth'
Push-Location $projOK8
try {
  Invoke-XX scopes list --order=desc
  Assert-Eq 'exit 0' $RunRC 0
  $expected = @("0002-bravo`tvalid`tauth", "0001-alpha`tvalid`tauth") -join "`n"
  Assert-Eq 'explicit desc matches default' $RunOut $expected
} finally { Pop-Location }

Start-Case 'scopes list --order=bogus rejected'
Push-Location $projOK8
try {
  Invoke-XX scopes list --order=bogus
  Assert-Eq       'exit 2'     $RunRC 2
  Assert-Contains 'diagnostic' $RunErr '--order must be'
} finally { Pop-Location }

Start-Case 'scopes list --system <id1>,<id2> OR semantics via comma list'
$projOK9 = New-FreshProject
Initialize-ProjectScaffold $projOK9
$staxDirOK9 = Join-Path $projOK9 $STAX_DIR
Write-Scope $staxDirOK9 '0001-a.md' 'valid' 'checkout-service'
Write-Scope $staxDirOK9 '0002-b.md' 'valid' 'payment-audit-log'
Write-Scope $staxDirOK9 '0003-c.md' 'valid' 'other-system'
Push-Location $projOK9
try {
  Invoke-XX scopes list --system 'checkout-service,payment-audit-log' --order=asc
  Assert-Eq 'exit 0' $RunRC 0
  $expected = @("0001-a`tvalid`tcheckout-service", "0002-b`tvalid`tpayment-audit-log") -join "`n"
  Assert-Eq 'comma-list OR' $RunOut $expected
} finally { Pop-Location }

Start-Case 'scopes list --system repeated flag matches comma form'
Push-Location $projOK9
try {
  Invoke-XX scopes list --system checkout-service --system payment-audit-log --order=asc
  Assert-Eq 'exit 0' $RunRC 0
  $expected = @("0001-a`tvalid`tcheckout-service", "0002-b`tvalid`tpayment-audit-log") -join "`n"
  Assert-Eq 'repeated-flag OR' $RunOut $expected
} finally { Pop-Location }

Start-Case 'scopes list --system matches any element of a multi-id systems array'
$projOK10 = New-FreshProject
Initialize-ProjectScaffold $projOK10
$staxDirOK10 = Join-Path $projOK10 $STAX_DIR
Write-Scope $staxDirOK10 '0001-multi.md' 'valid' 'checkout-service,payment-audit-log'
Write-Scope $staxDirOK10 '0002-other.md' 'valid' 'other-system'
Push-Location $projOK10
try {
  Invoke-XX scopes list --system payment-audit-log
  Assert-Eq 'exit 0' $RunRC 0
  Assert-Eq 'single-id flag matches multi-id row' $RunOut "0001-multi`tvalid`tcheckout-service,payment-audit-log"
} finally { Pop-Location }

# ---------- scopes lint: full per-check matrix ----------

# Writes a complete, lint-clean scope to .stax/name with the given status,
...
Start-Case 'scopes lint passes on a clean single-system scope'
...
Start-Case 'scopes lint passes on a clean multi-system scope'
...
Start-Case 'scopes lint flags a bad filename (non-conforming pattern)'
...
Start-Case 'scopes lint flags a missing _data_systems.yaml entry'
$projLNd = New-FreshProject
Initialize-ProjectScaffold $projLNd
# Registry has only Auth; scope references ghost.
...
Start-Case 'scopes lint flags an invalid status'
...
Start-Case 'scopes lint flags a missing title:'
...
Start-Case 'scopes lint flags an empty title:'
...
Start-Case 'scopes lint flags a missing created:'
...
Start-Case 'scopes lint flags a malformed created: timestamp'
...
Start-Case 'scopes lint flags date-only created: (no time component)'
...
Start-Case 'scopes lint flags title-not-first (frontmatter order)'
...
Start-Case 'scopes lint flags created-not-last (frontmatter order)'
...
Start-Case 'scopes lint flags dangling supersedes link'
...
Start-Case 'scopes lint flags dangling extends link'
...
Start-Case 'scopes lint rejects self-supersedes'
...
try {
  Invoke-XX scopes lint
  Assert-Eq       'exit 1'                   $RunRC 1
  Assert-Contains 'self-supersedes finding'  $RunOut 'cannot reference the scope itself'
} finally { Pop-Location }

Start-Case 'scopes lint rejects self-extends'
...
try {
  Invoke-XX scopes lint
  Assert-Eq       'exit 1'                 $RunRC 1
  Assert-Contains 'self-extends finding'   $RunOut 'cannot reference the scope itself'
} finally { Pop-Location }

Start-Case 'scopes lint flags missing extends back-link (bidirectional)'
...
# Scope A extends B. B exists but has no extended_by back-link to A.
...
try {
  Invoke-XX scopes lint
  Assert-Eq       'exit 1'                       $RunRC 1
  Assert-Contains 'missing back-link finding'    $RunOut 'does not list this scope in its `extended_by:`'
} finally { Pop-Location }

Start-Case 'scopes lint flags missing supersedes back-link (bidirectional)'
...
try {
  Invoke-XX scopes lint
  Assert-Eq       'exit 1'                       $RunRC 1
  Assert-Contains 'missing back-link finding'    $RunOut 'does not list this scope in its `superseded_by:`'
} finally { Pop-Location }

Start-Case 'scopes lint passes bidirectional supersedes when both sides linked'
...
Start-Case 'scopes lint flags EARS subject not in registry'
...
Start-Case 'scopes lint flags EARS-subject ↔ systems set divergence'
...
Start-Case 'scopes lint flags filename slug not matching slugify(title)'
...
Start-Case 'scopes lint flags missing required section ## Goal'
...
Start-Case 'scopes lint flags file exceeding max_scope_lines'
$projLNy = New-FreshProject
Initialize-ProjectScaffold $projLNy
# Pin max_scope_lines=15 so a 30-line file trips the cap.
$lockY = Join-Path (Join-Path $projLNy $STAX_DIR) $STAX_LOCK_FILE
Set-Content -LiteralPath $lockY -Value '{"prefix_width":4,"max_scope_lines":15,"review_per":"task"}' -Encoding ascii
...
Start-Case 'scopes lint flags missing frontmatter entirely'
...
Assert-Contains 'max-scope-lines listed' $RunErr '--max-scope-lines'
...
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 6 --max-scope-lines 30 --review-per task
  Assert-Eq 'exit 0' $RunRC 0
...
Start-Case 'init --max-scope-lines=50 seeds the lock with 50'
Reset-UserHome
$projF4 = New-FreshProject
Push-Location $projF4
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 4 --max-scope-lines 50 --review-per task
  Assert-Eq 'exit 0' $RunRC 0
  $lockContent = Get-Content -Raw -LiteralPath (Join-Path $projF4 $Script:STAX_LOCK_PATH)
  Assert-Contains 'lock has max_scope_lines=50' $lockContent '"max_scope_lines": 50'
} finally { Pop-Location }

Start-Case 'init --review-per=scope seeds the lock with scope'
Reset-UserHome
$projF5 = New-FreshProject
Push-Location $projF5
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 4 --max-scope-lines 30 --review-per scope
  Assert-Eq 'exit 0' $RunRC 0
  $lockContent = Get-Content -Raw -LiteralPath (Join-Path $projF5 $Script:STAX_LOCK_PATH)
  Assert-Contains 'lock has review_per=scope' $lockContent '"review_per": "scope"'
} finally { Pop-Location }
...
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 0 --max-scope-lines 30 --review-per task
  Assert-Eq       'exit 1'     $RunRC 1
  Assert-Contains 'diagnostic' $RunErr '--prefix-width must be positive'
} finally { Pop-Location }

Start-Case 'init --max-scope-lines=-5 is rejected'
Reset-UserHome
$projF9 = New-FreshProject
Push-Location $projF9
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 4 --max-scope-lines=-5 --review-per task
  Assert-Eq       'exit 1'     $RunRC 1
  Assert-Contains 'diagnostic' $RunErr '--max-scope-lines must be positive'
} finally { Pop-Location }
...
try {
  Invoke-XX init --scope project --agents claude `
                    --prefix-width 4 --max-scope-lines 30 --review-per ''
  Assert-Eq       'exit 1'     $RunRC 1
  Assert-Contains 'diagnostic' $RunErr 'invalid --review-per'
} finally { Pop-Location }
...
try {
  # agents=1,2 (both), scope=1 (project), prefix-width=(default), max-scope-lines=(default), review-per=(default=task)
  ...
  Assert-Contains 'default max_scope_lines' $lockI1 '"max_scope_lines": 30'
  Assert-Contains 'default review_per task' $lockI1 '"review_per": "task"'
} finally { Pop-Location }

Start-Case 'init interactive: review-per=2 (scope) chosen via prompt'
Reset-UserHome
...
try {
  ...
  Assert-Contains 'review_per scope via prompt' $lockI2 '"review_per": "scope"'
} finally { Pop-Location }
...
try {
  ...
  Assert-Contains 'max_scope_lines=42' $lockI3 '"max_scope_lines": 42'
} finally { Pop-Location }
...
try {
  ...
  Assert-Contains 'bundled hook landed'         $merged3 'stax scopes lint'
} finally { Pop-Location }
...
try {
  ...
  Assert-Contains 'bundled hook present'    $merged5 'stax scopes lint'
} finally { Pop-Location }
...
try {
  ...
  Assert-Contains 'bundled hook present before remove' $beforeContent 'stax scopes lint'
  Invoke-XX skills remove --project
  Assert-Eq    'remove exit 0' $RunRC 0
  $afterContent = Get-Content -Raw -LiteralPath $settingsPath
  Assert-NotContains 'bundled hook removed'  $afterContent 'stax scopes lint'
} finally { Pop-Location }
...
try {
  ...
  (Get-Content -Raw -LiteralPath $settingsPath2).Replace('stax scopes lint', 'stax scopes lint --verbose') |
    Set-Content -LiteralPath $settingsPath2 -Encoding ascii
  Invoke-XX skills remove --project
  Assert-Eq 'remove exit 0' $RunRC 0
  $after2 = Get-Content -Raw -LiteralPath $settingsPath2
  Assert-Contains 'tweaked hook survived' $after2 'stax scopes lint --verbose'
} finally { Pop-Location }
...
try {
  ...
  Assert-NotContains 'bundled hook removed'    $after3 'stax scopes lint'
} finally { Pop-Location }
...
Start-Case 'scopes list does NOT special-case hidden files (returns them in walk output)'
$projH = New-FreshProject
Initialize-ProjectScaffold $projH
$staxDirH = Join-Path $projH $STAX_DIR
Write-Scope $staxDirH '0001-keep.md' 'valid' 'auth'
# listPlans walks via os.ReadDir + a filename-regex match; it does not
# consult the Win32 hidden attribute. A user who hides a scope file for
# their own organizational reasons should still see it in `scopes list`,
# matching POSIX dotfile-handling semantics elsewhere in the CLI.
$hidden = Join-Path $staxDirH '0002-hidden.md'
Write-Scope $staxDirH '0002-hidden.md' 'valid' 'auth'
(Get-Item -LiteralPath $hidden).Attributes = 'Hidden'
Push-Location $projH
try {
  Invoke-XX scopes list
  Assert-Eq        'exit 0' $RunRC 0
  Assert-Contains  'visible scope listed' $RunOut '0001-keep'
  Assert-Contains  'hidden scope also listed' $RunOut '0002-hidden'
} finally { Pop-Location }

# ---------- Windows-specific: read-only files ----------

Start-Case 'scopes lint reads a read-only scope file successfully'
...
Start-Case 'scopes slugify handles a title with leading/trailing whitespace'
Invoke-XX scopes slugify '   Foo Bar   '
Assert-Eq 'exit 0' $RunRC 0
Assert-Eq 'whitespace stripped' $RunOut 'foo-bar'

Start-Case 'scopes slugify handles a title with embedded tab characters'
Invoke-XX scopes slugify "Foo`tBar`tBaz"
Assert-Eq 'exit 0' $RunRC 0
Assert-Eq 'tabs collapsed to single dash' $RunOut 'foo-bar-baz'

Start-Case 'scopes slugify handles a title with embedded newlines'
Invoke-XX scopes slugify "Foo`nBar"
Assert-Eq 'exit 0' $RunRC 0
Assert-Eq 'newline collapsed to single dash' $RunOut 'foo-bar'

Start-Case 'scopes slugify accepts a title with leading dashes after --'
Invoke-XX scopes slugify -- '-leading-dash-title'
Assert-Eq 'exit 0' $RunRC 0
Assert-Eq 'leading dashes trimmed' $RunOut 'leading-dash-title'

Start-Case 'scopes slugify drops non-ASCII characters'
Invoke-XX scopes slugify 'café Søk'
Assert-Eq 'exit 0' $RunRC 0
Assert-Eq 'non-ASCII collapsed' $RunOut 'caf-s-k'

Start-Case 'scopes slugify rejects wholly non-ASCII titles as unsluggable'
Invoke-XX scopes slugify '日本語'
Assert-Eq       'exit 2'     $RunRC 2
Assert-Contains 'diagnostic' $RunErr 'no slug-able characters'
...
try {
  ...
  Assert-NotContains 'no scopes-dir leak'  $RunErr $STAX_DIR
  Assert-NotContains 'no stax init mention is ok' $RunErr 'C:\'
} finally { Pop-Location }

# ---------- Windows-specific: lock-file parsing tolerates extra whitespace ----------

Start-Case 'scopes next-prefix tolerates pretty-printed _config.lock'
...
Start-Case 'scopes next-prefix falls back to default on malformed _config.lock'
...
Start-Case 'scopes next-prefix falls back when prefix_width is non-positive'
...
# ---------- Windows-specific: scopes list with no scopes returns empty ----------

Start-Case 'scopes list on fresh project returns empty output (no rows, no error)'
...
Start-Case 'scopes list --order=asc on fresh project returns empty'
Push-Location $projEmpty
try {
  Invoke-XX scopes list --order=asc
  Assert-Eq 'exit 0' $RunRC 0
...
Start-Case 'scopes lint on fresh project returns 0 with 0 ok'
Push-Location $projEmpty
try {
  Invoke-XX scopes lint
  Assert-Eq       'exit 0'                $RunRC 0
...
Start-Case 'scopes slugify produces a slug for a 200-char title'
$longTitle = ('Lorem ' * 40).TrimEnd()
Invoke-XX scopes slugify $longTitle
Assert-Eq 'exit 0' $RunRC 0
Assert-Contains 'slug starts with lorem' $RunOut 'lorem'

Start-Case 'scopes list ignores a filename with a 200-char slug if the prefix format is right'
...
# without it the Write-Scope call fails; in that case the test marks itself
...
$wroteLong = $false
try {
  Write-Scope $staxDirLong $longName 'valid' 'auth'
  $wroteLong = $true
} catch {}
if ($wroteLong) {
...
# ---------- scopes lint: more edge cases ----------

Start-Case 'scopes lint flags an empty systems array'
...
Start-Case 'scopes lint rejects block-form systems frontmatter'
...
Start-Case 'scopes lint flags a scope missing the ## Tasks section'
...
Start-Case 'scopes lint flags a scope missing the ## Approach section'
...
Start-Case 'scopes lint passes bidirectional extends when both sides linked'
...
Start-Case 'scopes lint accumulates multiple findings on one file'
...
Start-Case 'scopes lint reports per-file summary line on stderr'
...
# Wreck one scope with a known-bad status to force "1 ok, 1 failed".
...
try {
  ...
  Assert-Contains 'first scope ok line'   $RunOut '0001-foo.md: ok'
} finally { Pop-Location }

Start-Case 'scopes lint rejects positional arguments'
Push-Location $projSM
try {
  Invoke-XX scopes lint extra-arg
  Assert-Eq       'exit 2'        $RunRC 2
  Assert-Contains 'takes no args' $RunErr 'takes no arguments'
} finally { Pop-Location }

# ---------- scopes list: more filtering edge cases ----------

Start-Case 'scopes list --status nonexistent returns zero rows'
$projXS = New-FreshProject
Initialize-ProjectScaffold $projXS
Write-Scope (Join-Path $projXS $STAX_DIR) '0001-alpha.md' 'valid' 'auth'
Push-Location $projXS
try {
  Invoke-XX scopes list --status nope
  Assert-Eq 'exit 0'    $RunRC 0
  Assert-Eq 'no rows'   $RunOut ''
} finally { Pop-Location }

Start-Case 'scopes list keeps comma-list of statuses without duplicates'
$projDS = New-FreshProject
Initialize-ProjectScaffold $projDS
Write-Scope (Join-Path $projDS $STAX_DIR) '0001-alpha.md' 'valid' 'auth'
Write-Scope (Join-Path $projDS $STAX_DIR) '0002-bravo.md' 'valid' 'auth'
Push-Location $projDS
try {
  Invoke-XX scopes list --status 'valid,valid'
  Assert-Eq 'exit 0' $RunRC 0
  $expected = @("0002-bravo`tvalid`tauth", "0001-alpha`tvalid`tauth") -join "`n"
  Assert-Eq 'dup statuses collapsed' $RunOut $expected
} finally { Pop-Location }

Start-Case 'scopes list skips a scope with missing status:'
...
Write-Scope $staxDirMS '0002-ok.md' 'valid' 'auth'
Push-Location $projMS
try {
  Invoke-XX scopes list
  Assert-Eq       'exit 0'             $RunRC 0
  Assert-Eq       'only ok scope'       $RunOut "0002-ok`tvalid`tauth"
  Assert-Contains 'warning on broken'  $RunErr '0001-broken.md'
} finally { Pop-Location }

Start-Case 'scopes list skips a scope with missing systems:'
...
Write-Scope $staxDirMSY '0002-ok.md' 'valid' 'auth'
...
Start-Case 'scopes list emits status verbatim (does not normalize case)'
...
Write-Scope $staxDirCS '0001-alpha.md' 'valid' 'auth'
...
# ---------- scopes next-prefix: edge cases ----------

Start-Case 'scopes next-prefix ignores files whose prefix differs from configured width'
...
# 4-wide regex scan.
Write-Scope $staxDirWX '0003-three.md'  'valid' 'auth'
Write-Scope $staxDirWX '00099-extra.md' 'valid' 'auth'
...
Start-Case 'scopes next-prefix ignores directories that match the prefix format'
$projDX = New-FreshProject
Initialize-ProjectScaffold $projDX
$staxDirDX = Join-Path $projDX $STAX_DIR
Write-Scope $staxDirDX '0001-foo.md' 'valid' 'auth'
...
# silently ignored. Only 0001-foo.md remains as the recognized scope, so the
# next prefix is 0002.
...
try {
  ...
  Assert-Contains 'bundled hook present pre-remove' $beforeContent 'stax scopes lint'
  Invoke-XX skills remove --user
  Assert-Eq 'remove exit 0' $RunRC 0
...
Start-Case 'scopes list stdout has no UTF-8 BOM'
$projUB = New-FreshProject
Initialize-ProjectScaffold $projUB
Write-Scope (Join-Path $projUB $STAX_DIR) '0001-alpha.md' 'valid' 'auth'
Push-Location $projUB
try {
  $tmpStdout = [System.IO.Path]::GetTempFileName()
  try {
    & $Script:BuildBin scopes list > $tmpStdout
...
Start-Case 'scopes list uses LF line endings (not CRLF) on stdout'
Push-Location $projUB
try {
  Write-Scope (Join-Path $projUB $STAX_DIR) '0002-bravo.md' 'valid' 'auth'
  $tmpStdout2 = [System.IO.Path]::GetTempFileName()
  try {
    & $Script:BuildBin scopes list > $tmpStdout2
...
Start-Case 'scopes list stdout is exactly one trailing newline per row'
$projNL = New-FreshProject
Initialize-ProjectScaffold $projNL
$staxDirNL = Join-Path $projNL $STAX_DIR
Write-Scope $staxDirNL '0001-alpha.md' 'valid' 'auth'
Write-Scope $staxDirNL '0002-bravo.md' 'valid' 'auth'
Push-Location $projNL
try {
  $tmpStdout3 = [System.IO.Path]::GetTempFileName()
  try {
    & $Script:BuildBin scopes list > $tmpStdout3
...
Start-Case 'scopes next-prefix output is identical whether invoked via pwsh or cmd.exe'
$projXP = New-FreshProject
Initialize-ProjectScaffold $projXP
Write-Scope (Join-Path $projXP $STAX_DIR) '0007-foo.md' 'valid' 'auth'
Push-Location $projXP
try {
  $pwshOut = (& $Script:BuildBin scopes next-prefix).Trim()
  $cmdOut  = (& cmd.exe /c "`"$Script:BuildBin`" scopes next-prefix").Trim()
  Assert-Eq 'pwsh and cmd outputs match' $pwshOut $cmdOut
} finally { Pop-Location }

# ---------- Windows-specific: arg passing with quotes / spaces ----------

Start-Case 'scopes slugify handles quoted title with embedded spaces (pwsh)'
Invoke-XX scopes slugify 'Hello   World'
Assert-Eq 'exit 0' $RunRC 0
Assert-Eq 'multi-space slug' $RunOut 'hello-world'

Start-Case 'scopes slugify handles a title containing single quotes'
Invoke-XX scopes slugify "It's a Test"
Assert-Eq 'exit 0' $RunRC 0
Assert-Eq 'apostrophe collapsed to dash' $RunOut 'it-s-a-test'

Start-Case 'scopes slugify handles a title containing double quotes'
# In pwsh, double quotes inside single-quoted strings are literal.
Invoke-XX scopes slugify 'Quote "this" please'
Assert-Eq 'exit 0' $RunRC 0
Assert-Eq 'double-quotes collapsed' $RunOut 'quote-this-please'

Start-Case 'scopes slugify handles a title containing path separators'
Invoke-XX scopes slugify 'a\b/c'
Assert-Eq 'exit 0' $RunRC 0
...
Start-Case 'scopes list ignores .DS_Store-like cruft at the .stax/ root'
$projDS = New-FreshProject
Initialize-ProjectScaffold $projDS
$staxDirDSL = Join-Path $projDS $STAX_DIR
Write-Scope $staxDirDSL '0001-foo.md' 'valid' 'auth'
...
try {
  Invoke-XX scopes list
  Assert-Eq       'exit 0'                $RunRC 0
  Assert-Eq       'only matching scope'    $RunOut "0001-foo`tvalid`tauth"
...
# ---------- Windows-specific: scopes next-prefix never panics on empty proj dir ----------

Start-Case 'scopes next-prefix returns 0001 when .stax/ has only the scaffold files'
