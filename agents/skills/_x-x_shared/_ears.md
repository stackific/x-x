...
1. **Exactly one named system per criterion.** The system is a concrete subsystem, service, or component from the registry (`<cwd>/.x-plans/_data_systems.yaml`) — never bare `the system`, `it`, `the app`, `the service`, `the application`, `the platform`. See `../_x-x_shared/_systems.md` for registry consultation and propose-new-system rules.
