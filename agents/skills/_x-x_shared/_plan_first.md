...
- **Named systems used or proposed:** which entries from `<cwd>/.x-plan/_data_systems.yaml`'s `systems` array each criterion targets, and any new systems that will be added (with `name` and `brief`).
...
- `<cwd>/.x-plan/_data_systems.yaml` — registry of named systems (id, name, brief).
...
Every plan lives at `<cwd>/.x-plan/<prefix>-<slug>.md` where:

- `<prefix>` is a zero-padded numeric prefix returned by `x-x plan next-prefix`. The current width is set by `prefix_width` in `<cwd>/.x-plan/_config.lock` (seeded by `x-x init`; defaults to 4).
...
`x-x plan lint` enforces, for **both** forward/back pairs (`supersedes`↔`superseded_by`, `extends`↔`extended_by`): every slug resolves to a sibling plan; self-references are rejected; every forward link has a matching back link and vice versa.
...
- `x-x plan slugify "<title>"` — prints the kebab-case slug for the given title. Use it to derive the post-prefix portion of new plan filenames so author and lint agree on the same algorithm.

All Go commands except `slugify` read width/line-cap from `<cwd>/.x-plan/_config.lock` (seeded by `x-x init`). Files with missing or malformed frontmatter trigger stderr warnings in `x-x plan list` and are reported as findings by `x-x plan lint`.
