...
A typical `.x-plan/_data_systems.yaml` should read:
...
Other skills read `.x-plan/_data_systems.yaml` directly and update it as needed. 
...
For each spec or task the skills intend to write, they:
   ...
   3. If matched: use the existing entry's `name` verbatim in the criterion (e.g., "the Checkout Service shall …").
   4. If not matched: STOP. Propose a new system to the user (name + one-sentence brief). On approval, add that to `.x-plan/_data_systems.yaml`. Then continue.

## Source of truth

A system's current contract is the set of `[x]` EARS criteria across plans whose frontmatter is `status: valid` AND whose `systems:` array includes the system. Use `x-x plan list --status valid --system <Name>` to enumerate them, then read each plan's `## Tasks` for `[x]` criteria naming the system. Plans with `status: superseded` or `status: deprecated` are history and must never be read for current truth.
...
- Never write the slug into EARS criterion text. Use the display name (`"the Checkout Service"`) — the slug is for internal use only.
